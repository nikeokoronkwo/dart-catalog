extension IterbaleExtensions<T> on Iterable<T> {
  Iterable<T> applyWhere(
      bool Function(T element) test, T Function(T element) apply) {
    return this.map((e) {
      if (test(e))
        return apply(e);
      else
...
extension StringIterableExtensions on Iterable<String> {
  ...
  Iterable<String> splitMapJoinWhere(
      Pattern separator, bool Function(String element) test,
      {int? start, int? end}) {
    return this;
  }
}
