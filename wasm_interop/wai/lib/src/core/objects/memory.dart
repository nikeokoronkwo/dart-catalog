...
extension type WebAssemblyMemory._(JSObject _) implements JSObject {
  ...
  factory WebAssemblyMemory.instantiate({required int initial, int? maximum, bool? shared})
    => WebAssemblyMemory(WebAssemblyMemoryDescriptor(initial: initial, maximum: maximum, shared: shared));
...
extension type WebAssemblyMemoryDescriptor._(JSObject _) implements JSObject {
  external WebAssemblyMemoryDescriptor({int initial, int? maximum, bool? shared});
